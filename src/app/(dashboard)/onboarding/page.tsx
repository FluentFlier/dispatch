...
export default function OnboardingPage() {
  ...
  const inputCls =
    'w-full bg-[#F4F2EF] border-[0.5px] border-[#1A1714]/12 rounded-[7px] px-4 py-3 font-body text-[13px] text-[#1A1714] placeholder:text-[#8C857D] focus:outline-none focus:border-[#1A1714]/40 transition-colors';
  const textareaCls = `${inputCls} resize-none`;
  const labelCls = 'block font-body text-[13px] text-[#4A4540] mb-2';
      ...
      case 0:
        return (
          <div className="space-y-5">
            <div>
              <h2 className="font-display font-[700] text-[20px] text-[#1A1714] mb-1">
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-4">
              {pillars.map((pillar, i) => (
                <div
                  key={i}
                  className="border-[0.5px] border-[#1A1714]/12 rounded-[12px] p-4 space-y-3"
                  style={{ borderLeftColor: pillar.color, borderLeftWidth: 3 }}
                >
                  <div className="flex items-center justify-between">
                    <span className="font-body text-[12px] font-medium text-[#8C857D]">
                      Pillar {i + 1}
                    </span>
                    {pillars.length > 1 && (
                      <button
                        ...
                        className="text-[11px] text-[#8C857D] hover:text-[#EB5E55] transition-colors"
                      >
                        Remove
                      </button>
                    )}
                      ...
                      {PRESET_COLORS.map((color) => (
                        <button
                          ...
                          className={`w-6 h-6 rounded-full transition-transform ${
                            pillar.color === color
                              ? 'ring-2 ring-[#1A1714] ring-offset-1 ring-offset-[#FAFAF8] scale-110'
                              : 'hover:scale-110'
                          }`}
              ...
              {pillars.length < 6 && (
                <button
                  type="button"
                  onClick={addPillar}
                  className="w-full border-[0.5px] border-dashed border-[#1A1714]/12 rounded-[12px] py-3 text-[13px] text-[#8C857D] hover:border-[#EB5E55] hover:text-[#EB5E55] transition-colors"
                >
                  + Add pillar
                </button>
              )}
  ...
  return (
    <div className="max-w-lg mx-auto py-12 px-4">
      <h1 className="font-display font-[800] text-[18px] text-[#1A1714] tracking-[0.16em] mb-2">
        DISPATCH
      </h1>
      <p className="font-body text-[13px] text-[#8C857D] mb-6">
      ...
      {error && (
        <p className="font-body text-[13px] text-[#EB5E55] mt-4">{error}</p>
      )}
      ...
      <div className="flex items-center justify-between mt-8">
        {step > 0 ? (
          <button
            type="button"
            onClick={handleBack}
            className="rounded-[7px] py-[10px] px-[20px] font-body text-[13px] font-medium text-[#4A4540] border-[0.5px] border-[#1A1714]/12 hover:border-[#1A1714]/25 transition-colors"
          >
            Back
          </button>
        ) : (
        ...
        {isLastStep ? (
          <button
            ...
            disabled={loading}
            className="rounded-[7px] py-[10px] px-[24px] text-[#FAFAF8] font-body text-[13px] font-medium bg-[#EB5E55] hover:opacity-90 transition-all duration-100 disabled:opacity-40"
          >
