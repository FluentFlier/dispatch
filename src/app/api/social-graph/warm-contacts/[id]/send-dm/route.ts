...
export async function POST(
  ...
  try {
    ...
    if (typeof body?.message === 'string') messageOverride = body.message;
  } catch {
    // empty body ok
  }
  ...
  try {
    const result = await sendWarmContactDm(client, workspaceId, user.id, params.id, {
      messageOverride,
    });
    ...
    console.error('[social-graph/warm-contacts/send-dm]', err);
    return NextResponse.json(
