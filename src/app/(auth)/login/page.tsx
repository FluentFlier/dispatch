...
export default function LoginPage() {
  ...
  async function handleAuth() {
    ...
    // Check for insforge_code (OAuth callback)
    const params = new URLSearchParams(window.location.search);
    const hasCode = params.has("insforge_code");

    if (hasCode) {
      setStatus("Completing sign-in...");
      // Wait for SDK constructor to process the code
      // The SDK's detectAuthCallback runs async in constructor
      for (let i = 0; i < 10; i++) {
        ...
        try {
          ...
          if (data?.user) {
            ...
            // Get the access token and sync to cookie
            const token = (client.auth as unknown as { getAccessToken(): string | null }).getAccessToken();
            if (token) {
      ...
      // If we get here, exchange failed
    ...
    // No callback -- check existing session
  ...
  return (
    <div ref={spotRef} className="relative min-h-screen overflow-hidden" style={{ background: '#050507', fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        @keyframes glow-pulse { 0%, 100% { box-shadow: 0 0 20px rgba(129,140,248,0.1); } 50% { box-shadow: 0 0 30px rgba(129,140,248,0.15); } }
      `}</style>

      <div className="fixed inset-0 pointer-events-none" style={{ backgroundImage: 'radial-gradient(rgba(255,255,255,0.06) 1px, transparent 1px)', backgroundSize: '24px 24px' }} />
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-[200px] -left-[100px] w-[500px] h-[500px] rounded-full opacity-[0.1]" style={{ background: 'radial-gradient(circle, #6366F1, transparent 70%)', filter: 'blur(100px)', animation: 'float1 12s ease-in-out infinite alternate' }} />
        <div className="absolute -bottom-[150px] -right-[100px] w-[400px] h-[400px] rounded-full opacity-[0.07]" style={{ background: 'radial-gradient(circle, #8B5CF6, transparent 70%)', filter: 'blur(120px)', animation: 'float2 15s ease-in-out infinite alternate' }} />
      </div>
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ background: 'radial-gradient(600px circle at var(--mx, 50%) var(--my, 50%), rgba(99,102,241,0.07), transparent 50%)' }} />
        ...
        {!ready ? (
          ...
          <div className="w-full max-w-[380px]" style={{ animation: 'fadeUp 0.6s ease-out' }}>
            <div className="text-center mb-10">
              <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl mb-5" style={{ background: 'rgba(129,140,248,0.1)', border: '1px solid rgba(129,140,248,0.15)' }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#818CF8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>
              </div>
              <h1 style={{ fontFamily: "'Instrument Serif', serif", fontSize: '28px', color: '#FAFAFA', fontWeight: 400, letterSpacing: '-0.02em' }}>Welcome to Dispatch</h1>
              <p className="text-[14px] text-[#71717A] mt-2">Sign in to your content command center</p>
            </div>

            <div className="rounded-2xl p-8" style={{ background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)', backdropFilter: 'blur(12px)', animation: 'glow-pulse 3s ease-in-out infinite' }}>
              ...
              {error && (
                <div className="mt-4 px-3 py-2 rounded-lg text-[13px] text-[#FCA5A5]" style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.15)' }}>{error}</div>
              )}
            ...
            <p className="text-center text-[11px] text-[#52525B] mt-6">By continuing, you agree to our terms of service.</p>
          </div>
        )}
      </div>
    </div>
