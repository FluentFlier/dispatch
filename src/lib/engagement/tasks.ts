 ...
 *
 * Ported from imagine's task-queue architecture: lease-based locking so
 * overlapping cron runs never double-post, bounded attempts, per-account
 * daily caps, and human-mimicking delays between actions — LinkedIn restricts
 * accounts whose activity looks robotic.
 */
...
/**
 * Processes due approved tasks. Claim protocol (imagine's lease pattern):
