...
export async function processIngestedPosts(
  ...
  for (const post of fresh) {
    ...
    try {
      // Raw post kept for cross-run dedupe; the signal itself lands on the
      // matching LEAD (intent flags + event trail + Slack) - the separate
      // signal-events feed is retired.
      ...
      result.signalsCreated += bridge.matched + bridge.created;
    } catch (err) {
...
/** Ingest a single post (webhook / manual seed) - the signal lands on the lead. */
export async function ingestSinglePost(
  ...
  await upsertRawPost(client, workspaceId, sourceId, post);
  const bridge = await applySignalToLeads(client, workspaceId, classified, {
    sourceUrl: post.postUrl,
  });
  return { created: bridge.matched + bridge.created > 0 };
}
