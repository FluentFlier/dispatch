  ...
  return (
    <section className="bg-[#FAFAF8] border-[0.5px] border-[rgba(26,23,20,0.12)] rounded-[12px] p-[13px_14px]">
      <div className="flex items-center justify-between mb-3">
        <p className="font-['Space_Grotesk'] font-medium text-[10px] uppercase tracking-[0.10em] text-[#8C857D]">
          TODAY&apos;S PROMPT
        </p>
        <button
          onClick={fetchSuggestion}
          disabled={loading}
          className="text-[#8C857D] hover:text-[#EB5E55] transition-colors duration-100 disabled:opacity-50"
      ...
      {loading ? (
        <div className="space-y-2">
          <div className="h-3 w-full bg-[#EDECEA] rounded animate-pulse" />
          <div className="h-3 w-3/4 bg-[#EDECEA] rounded animate-pulse" />
          <div className="h-3 w-5/6 bg-[#EDECEA] rounded animate-pulse" />
        </div>
      ) : (
        <p className="font-['Space_Grotesk'] text-[13px] text-[#1A1714] leading-[1.55]">
          {suggestion}
        </p>
      )}
    </section>
